...
export class SupabaseService {
  ...
  async obtenerTodasFacturas() {
    try {
      const { data: facturas, error } = await this.supabase
        .from('facturas')
        .select('*')
        .order('fecha', { ascending: false });
  ...
  async obtenerFacturasUltimaCajaCerrada() {
    try {
      // 1. Buscar la última caja cerrada
      const { data: sesiones, error: errorSesion } = await this.supabase
        .from('caja_sesiones')
      ...
      if (errorSesion) throw errorSesion;
      if (!sesiones || sesiones.length === 0) {
        return { error: 'No hay sesiones cerradas' };
      }

      const { hora_apertura, hora_cierre } = sesiones[0];

      // 2. Buscar facturas en el rango de esa sesión
      const { data: facturas, error: errorFacturas } = await this.supabase
        .from('facturas')
        .select('*')
        .gte('fecha', hora_apertura)
        .lte('fecha', hora_cierre)
        .order('fecha', { ascending: false });
      ...
      console.error('❌ Error al obtener facturas de la última caja cerrada:', error);
      return { error: error.message };
    }
  }
  ...
  async obtenerCajaAbierta() {
    const { data, error } = await this.supabase
      ...
      .select('*')
      .eq('estado', 'abierta')
      .order('id', { ascending: false }) // más reciente primero
      .limit(1);
  ...
  async obtenerCajacerrada(): Promise<any> {
    const { data, error } = await this.supabase
      .from('caja_sesiones')
      .select('*')
      .eq('estado', 'cerrada')
    ...
    if (error) {
      console.error('Error al obtener caja cerrada:', error);
      return null;
    }
  ...
  async crearReservacion(reservacion: {
    ...
    try {
      ...
      console.error('Excepción al crear reservación:', error);
      return { error: error.message };
    }
  }
  async obtenerReservaciones() {
    try {
      ...
      console.error('Excepción al obtener reservaciones:', error);
      return { error: error.message };
    }
  }
  async editarReservacion(id: string, reservacion: {
    nombre_cliente?: string;
    motivo?: string;
    mesa_id?: string;
    personas?: number;
    ...
    try {
      ...
      console.error('Excepción al editar reservación:', error);
      return { error: error.message };
    }
  }
  async confirmarReservacion(id: string) {
    return await this.editarReservacion(id, { estado: 'confirmado' });
  }
  async cancelarReservacion(id: string) {
    return await this.editarReservacion(id, { estado: 'cancelado' });
  }
  /**Obtiene reservaciones filtradas por rango de fechas del evento (no de creación)*/
  async obtenerReservacionesPorFechaEvento(fechaInicio: string | Date, fechaFin: string | Date) {
    try {
      // Asegurar el formato correcto de las fechas
      const inicio = typeof fechaInicio === 'string' ? fechaInicio : fechaInicio.toISOString().split('T')[0];
      const fin = typeof fechaFin === 'string' ? fechaFin : fechaFin.toISOString().split('T')[0];

      const { data, error } = await this.supabase
        .from('reservaciones')
        .select('*')
        .gte('fecha', inicio)  // Fecha mayor o igual que inicio
        .lte('fecha', fin)    // Fecha menor o igual que fin
        .order('fecha', { ascending: true })  // Ordenar por fecha ascendente
        .order('hora', { ascending: true });  // Y luego por hora
      ...
      console.error('Excepción al obtener reservaciones por fecha:', error);
      return { error: error.message };
    }
  }
  // facutras con filtros para reportes
  async obtenerFacturasConFiltros(filtros: {
    tipoPeriodo: string,
    fecha?: string,            // Formato esperado: 'YYYY-MM-DD'
    mes?: number,
    anio?: number,
    semanaSeleccionada?: string, // formato "YYYY-MM-DD_YYYY-MM-DD"
  }) {
    // Función auxiliar para formatear fechas en formato Supabase (sin timezone)
    const formatSupabaseDate = (date: Date): string => {
    ...
    try {
      ...
      if (filtros.tipoPeriodo === 'dia' && filtros.fecha) {
        // Solución definitiva para timestamp without time zone
        const fechaInicio = `${filtros.fecha} 00:00:00`;
        const fechaFin = `${filtros.fecha} 23:59:59`;

        query = query
          .gte('fecha', fechaInicio)
          .lte('fecha', fechaFin);

        console.log('Filtro por día:', {
          fechaInicio,
          fechaFin,
          tipo: 'timestamp without time zone'
        });

      } else if (filtros.tipoPeriodo === 'semana' && filtros.semanaSeleccionada) {
        const [inicio, fin] = filtros.semanaSeleccionada.split('_');

        query = query
          .gte('fecha', `${inicio} 00:00:00`)  // Desde el inicio del primer día
          .lte('fecha', `${fin} 23:59:59`);    // Hasta el final del último día

        console.log('Consulta por semana:', {
          gte: `${inicio} 00:00:00`,
          lte: `${fin} 23:59:59`
        });


      } else if (filtros.tipoPeriodo === 'mes' && filtros.mes && filtros.anio) {
        const fechaInicio = new Date(filtros.anio, filtros.mes - 1, 1);
        const fechaFin = new Date(filtros.anio, filtros.mes, 0);

        query = query
          .gte('fecha', formatSupabaseDate(fechaInicio))
          .lte('fecha', `${fechaFin.getFullYear()}-${(fechaFin.getMonth() + 1).toString().padStart(2, '0')}-${fechaFin.getDate().toString().padStart(2, '0')} 23:59:59`);

      } else if (filtros.tipoPeriodo === 'anio' && filtros.anio) {
        const fechaInicio = new Date(filtros.anio, 0, 1);
        const fechaFin = new Date(filtros.anio, 11, 31);
  ...
  async verificarFacturasDirectamente(fecha: string) {
    try {
      // Consulta directa con el mismo formato que tu consulta manual
      const { data, error } = await this.supabase
        .from('facturas')
        .select('*')
        .gte('fecha', `${fecha} 00:00:00`)
        .lt('fecha', `${fecha} 23:59:59`);

      console.log('Resultado verificación directa:', {
        filtro: `fecha >= '${fecha} 00:00:00' AND fecha < '${fecha} 23:59:59'`,
        resultados: data,
        error
      });
      ...
      console.error('Error en verificación directa:', e);
      throw e;
    }
  }
  //prdcutos con filtro
  async obtenerProductosFiltrados(filtros: {
    categoriaId?: string;
    estadoStock?: 'critico' | 'agotado' | 'suficiente' | 'todos';
    soloActivos?: boolean;
  }) {
    // Primero obtenemos todos los productos no eliminados
    ...
    // Aplicamos el filtro de stock en el frontend
  ...
  async crearProveedor(proveedor: {
    ...
    try {
      ...
      console.error('Excepción al crear proveedor:', error);
      return { error: error.message };
    }
  }
  ...
  async obtenerGastosConFiltros(filtros: {
    tipoPeriodo: string,
    fecha?: string,
    mes?: number,
    anio?: number,
    ...
    try {
      ...
      // Solo categoría 'salarios'
      query = query.eq('categoria', 'salarios');

      // Filtro por nombre parcial de proveedor (si se proporciona)
      ...
      // Filtro por fecha
      if (filtros.tipoPeriodo === 'dia' && filtros.fecha) {
        const inicio = `${filtros.fecha} 00:00:00`;
        const fin = `${filtros.fecha} 23:59:59`;
        query = query.gte('fecha', inicio).lte('fecha', fin);

      } else if (filtros.tipoPeriodo === 'semana' && filtros.semanaSeleccionada) {
        const [inicio, fin] = filtros.semanaSeleccionada.split('_');
        query = query.gte('fecha', `${inicio} 00:00:00`).lte('fecha', `${fin} 23:59:59`);

      } else if (filtros.tipoPeriodo === 'mes' && filtros.mes && filtros.anio) {
        const inicio = new Date(filtros.anio, filtros.mes - 1, 1);
        const fin = new Date(filtros.anio, filtros.mes, 0);
        query = query
          .gte('fecha', formatSupabaseDate(inicio))
          .lte('fecha', `${fin.getFullYear()}-${(fin.getMonth() + 1).toString().padStart(2, '0')}-${fin.getDate().toString().padStart(2, '0')} 23:59:59`);

      } else if (filtros.tipoPeriodo === 'anio' && filtros.anio) {
        const inicio = new Date(filtros.anio, 0, 1);
        const fin = new Date(filtros.anio, 11, 31);
        query = query
          .gte('fecha', formatSupabaseDate(inicio))
          .lte('fecha', `${fin.getFullYear()}-${(fin.getMonth() + 1).toString().padStart(2, '0')}-${fin.getDate().toString().padStart(2, '0')} 23:59:59`);
      }
  ...
  async obtenerGastosConFiltrostotal(filtros: {
    tipoPeriodo: string,
    fecha?: string,
    mes?: number,
    anio?: number,
  ...
  async obtenerPedidosConDetalleConFiltros(filtros: {
    tipoPeriodo: string,
    fecha?: string,            // Formato esperado: 'YYYY-MM-DD'
    mes?: number,
    anio?: number,
